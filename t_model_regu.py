...
class LinearRegressionModelWithRegularization:
    def __init__(self, dims, learning_rate=0.05, n_estimators=100, batch_size=-1, lambd=0.25):
        self.dims = dims
    ...
    def update_params(self, grads, X):
    ...
    def compute_cost(self, y, y_hat):
        m = y.shape[0]
        cost = 1/m * np.sum((y_hat - y)**2) + self.lambd * np.sum(np.square(self.params['W']))
        return cost
    ...
    def batch_fit(self, X, y):
        for i in range(self.n_estimators):
            ...
            self.params = self.update_params(grads, X)
